import { PageMeta } from "@lib/types";

const pageMeta: PageMeta = {
  blogs: {
    title: "Blogs",
    description:
      "I've been writing online since 2021, mostly about web development and tech careers. In total, I've written more than 50 articles till now.",
    image: "https://gauravgola.in/og-img/lyra.png",
    keywords: " blog, blog, webdev, react",
  },

  bookmark: {
    title: "Bookmarked Blogs",
    description: "Bookmarked Blogs of Gaurav Gola's blogs by you",
    image: "https://gauravgola.in/og-img/lyra.png",
    keywords: "bookmark, blogs, ",
  },
  about: {
    title: "About",
    description:
      "Hey, I am Gaurav Gola. A Front-end Developer/React Developer from India who loves to design and code. I use React.js or Next.js to build the web application interfaces and the functionalities. At the moment, I am pursuing my Bachelor's degree in Computer Science.",
    image: "https://gauravgola.in/og-img/about.png",
    keywords: "about, about me, ",
  },
  snippets: {
    title: "Code Snippets",
    description:
      "These are a collection of code snippets I've used in the past and saved. These could be useful to you as well.",
    image: "https://gauravgola.in/og-img/lyra.png",
    keywords: "Code, Code Snippets, Snippets",
  },
};

export default pageMeta;
